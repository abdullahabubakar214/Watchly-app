...
const updateCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!coverImage.url) {
    throw new ApiError(
      400,
      "Error while Updating CoverImage file on cloudinary",
    );
  }
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    ...
    // it should typically be the last stage in your pipeline, used to specify which fields to return to the client.
    {
      $project: {
        fullName: 1,
        username: 1,
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, channel[0], "User Channel Count Successfully"));
});
    ...
    },
    {
      $lookup: {
        ...
        pipeline: [
          ...
          {
            // give the first value bz lookup give array
            $addFields: {
              owner: {
                $first: "$owner",
              },
